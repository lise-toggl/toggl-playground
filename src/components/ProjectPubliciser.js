...
export default class ProjectPubliciser extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ready: false,
  ...
  makePublic = () => {
    ...
    let project_batches = [];
    for (let i = 0; i < this.state.projects.length; i += 20) {
      project_batches.push(this.state.projects.slice(i, i + 20));
    }
    console.log(project_batches);
    const baseUrl = `/v9/workspaces/${this.state.ws}/projects/`;
    const data = [{ op: "replace", path: "/is_private", value: false }];
    project_batches.forEach(batch => {
      let url = baseUrl;
      ...
      console.log(url);
      limiter
        .schedule(() => TOGGL_API.patch(url, data))
